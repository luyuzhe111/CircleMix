...
output_dir = os.path.join(args.output_csv_dir, 'config', args.expname)
...
def main():
    ...
    # testing
    df = pd.DataFrame(columns=['exp', 'train', 'val', 'test', 'test_loss', 'test_acc', 'precision', 'recall', 'f1'])
    ...
    test_loss, test_acc, precision, recall, f1 = validate(output_dir,
                                                          val_loader,
                                                          model,
                                                          criterion,
                                                          use_cuda,
    ...
    df.loc[len(df)] = [expname, train_set, val_set, test_set, test_loss, test_acc, precision, recall, f1]
...
def validate(out_dir, valloader, model, criterion, use_cuda, mode):
    ...
    with torch.no_grad():
        ...
        precision = precision_score(target_history, pred_history, average='binary')
        recall = recall_score(target_history, pred_history, average='binary')
