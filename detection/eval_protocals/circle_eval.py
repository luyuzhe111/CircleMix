...
class CIRCLEeval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm', imgIds=[]):
        '''
        ...
        if not cocoGt is None:
            self.params.imgIds = sorted(cocoGt.getImgIds(imgIds))
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.iouThrs = np.linspace(.5, 0.9, int(np.round((0.9 - .5) / .05)) + 1, endpoint=True)
        # self.recThrs = np.linspace(.0, 1.00, int(np.round((1.00 - .0) / .01)) + 1, endpoint=True)
        self.recThrs = np.linspace(.0, 1.00, int(np.round((1.00 - .0) / .01)) + 1, endpoint=True)
        self.maxDets = [1, 500, 1000]
        self.areaRng = [[0 ** 2, 1e5 ** 2], [0 ** 2, 3.14* (((201-28)/2/0.25) ** 2)], [3.14* (((201-28)/2/0.25) ** 2), 3.14* (((201+28)/2/0.25) ** 2)], [3.14* (((201+28)/2/0.25) ** 2), 1e5 ** 2]]

        # self.areaRng = [[0 ** 2, 1e5 ** 2], [0 ** 2, 320 ** 2], [320 ** 2, 960 ** 2], [960 ** 2, 1e5 ** 2]]
