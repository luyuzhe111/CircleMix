...
def main():
    ...
    if args.num_classes < 5:
        ...
        parent_config = config.split(hierarchy)[0] + config.split(hierarchy)[1]
        parent_config_dir = os.path.join(os.path.dirname(config_dir), parent_config)
        parent_args = cfg_from_file(parent_config_dir)
        parent_args = Namespace(**parent_args)
        args.test_list = parent_args.test_list
